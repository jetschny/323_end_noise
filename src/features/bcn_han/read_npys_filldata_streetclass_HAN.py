...
in_file1="OSM_roads_han_streetclass_clip.npy"
in_file2="OSM_roads_han_nlanes_clip.npy"
in_file3="OSM_roads_han_maxspeed_clip.npy"

out_file2="OSM_roads_han_nlanes_clipfill.npy"
out_file3="OSM_roads_han_maxspeed_clipfill.npy"

grid1=np.load(base_in_folder+in_file1)
grid2=np.load(base_in_folder+in_file2)
grid3=np.load(base_in_folder+in_file3)
...
if write_switch:
    ...
    np.save(base_in_folder+out_file2,grid_target_export)
    
    grid_target_export=np.empty(grid1.shape)*0
    indexxy = np.where(grid1)
    grid_target_export[indexxy] = np.array(df["SpeedLimit"])
    np.save(base_in_folder+out_file3,grid_target_export)
